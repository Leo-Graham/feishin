...
const FADE_STEPS = 10; // Number of steps in the fade

const fadeVolume = async (start: number, end: number, duration: number) => {
    const step = (end - start) / FADE_STEPS;
    const stepDuration = duration / FADE_STEPS;

    // eslint-disable-next-line no-plusplus
    for (let i = 0; i < FADE_STEPS; i++) {
        const volume = start + step * (i + 1);
        await getMpvInstance()?.volume(volume);
...
ipcMain.on('player-pause', async () => {
    try {
        const currentVolume = (await getMpvInstance()?.getProperty('volume')) as unknown as number;
        await fadeVolume(currentVolume, 0, FADE_DURATION);
        await getMpvInstance()?.pause();
        await getMpvInstance()?.volume(currentVolume);
    } catch (err: NodeMpvError | any) {
        mpvLog({ action: 'Failed to pause mpv playback with fade' }, err);
    }
});
